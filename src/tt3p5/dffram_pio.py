...
sm = StateMachine(0, dffram_prog, out_base=Pin(9), sideset_base=Pin(0), in_base=Pin(3))
sm.active(1)

sm_rx = StateMachine(1, read_prog, in_base=Pin(3))
...
def linear_test():
    ...
    for i in range(RAM_LEN):
        addr = i
        val = (dst_data[RAM_LEN*2+i*2+0] & 0x3) | ((dst_data[RAM_LEN*2+i*2+0] >> 2) & 0xC) | ((dst_data[RAM_LEN*2+i*2 + 1] << 2) & 0xF0)
...
def random_test(runs):
    ...
    for i in range(RAM_LEN):
        val = (dst_data[RAM_LEN*2 + i*4+2] & 0x3) | ((dst_data[RAM_LEN*2 + i*4+2] >> 2) & 0xC) | ((dst_data[RAM_LEN*2 + i*4+3] << 2) & 0xF0)
        if val != 0:
            print("Clear failed")
            return 1
    ...
    for r in range(runs):
        ...
        for i in range(len(dst_data) // 4):
            ...
            if write:
                ...
                addr = src_data[i*4+2]
                val = (dst_data[i*4+2] & 0x3) | ((dst_data[i*4+2] >> 2) & 0xC) | ((dst_data[i*4 + 3] << 2) & 0xF0)
                expected = ram[addr]
                if val != expected:
                    print(f"Error, at {addr} read {val}, expected {expected}")
                    errors += 1
...
def test_freq_range(start_freq, end_freq=266_000_000, freq_step = 4_000_000, runs=20):
    try:
        ...
        while next_freq <= end_freq:
            if next_freq > 266_000_000:
                # Required to allow frequencies above 266MHz - needs
